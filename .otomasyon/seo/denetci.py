...
def sayfalar():
    ...
    l = [f for f in sorted(glob.glob("**/*.html", recursive=True)) if f not in ("admin.html", "404.html", "trend/404.html") and not f.startswith("onizleme/")]
    os.chdir(eski)
    return l
...
def denetle():
    ...
    sayfa_listesi = [f for f in sayfa_listesi if f not in ("admin.html", "404.html", "trend/404.html") and not f.startswith("onizleme/")]  # onizleme/: müşteriye özel gizli teklif sayfaları
